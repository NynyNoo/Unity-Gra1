...
        GUI.DrawTexture(new Rect(ResourceManager.Padding, ResourceManager.Padding, ResourceManager.HeaderWidth, ResourceManager.HeaderHeight), header);
        float leftPos = (header.width / 2) - ResourceManager.Padding*4;
        float topPos = 2 * ResourceManager.Padding + (header.height/2);
        GUI.Label(new Rect(leftPos, topPos, ResourceManager.MenuWidth - 2 * ResourceManager.Padding, ResourceManager.TextHeight), "Witaj " + PlayerManager.GetPlayerName());
...
    }
    ...
    protected virtual float GetMenuHeight()
    {
        ...
        if (buttons != null) buttonHeight = buttons.Length * 2 * ResourceManager.ButtonHeight;
